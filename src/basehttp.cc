...
bool RequestState::HeaderLine(string const &Line) /*{{{*/
{
	...
	if (Result == 0 && Line.size() > 4
			&& stringcasecmp(Line.data(), Line.data() + 4, "HTTP") == 0) {
		// Evil servers return no version
		if (Line[4] == '/') {
			int const elements = sscanf(Line.c_str(),
					"HTTP/%3u.%3u %3u%359[^\n]", &Major, &Minor, &Result, Code);
			if (elements == 3) {
				Code[0] = '\0';
				if (Owner != NULL && Owner->Debug == true)
					clog << "HTTP server doesn't give Reason-Phrase for "
							<< std::to_string(Result) << std::endl;
			} else if (elements != 4)
				return _error->Error(
						_("The HTTP server sent an invalid reply header"));
		} else {
			Major = 0;
			Minor = 9;
	...
	if (stringcasecmp(Tag, "Content-Length:") == 0) {
		...
		*DownloadSizePtr = ContentLength;
		if (*DownloadSizePtr >= std::numeric_limits<unsigned long long>::max())
			return _error->Errno("HeaderLine",
					_("The HTTP server sent an invalid Content-Length header"));
	...
	// for such responses.
	if ((Result == 416 || Result == 206)
			&& stringcasecmp(Tag, "Content-Range:") == 0) {
		HaveContent = true;

		// §14.16 says 'byte-range-resp-spec' should be a '*' in case of 416
		if (Result == 416
				&& sscanf(Val.c_str(), "bytes */%llu", &TotalFileSize) == 1)
			; // we got the expected filesize which is all we wanted
		else if (sscanf(Val.c_str(), "bytes %llu-%*u/%llu", &StartPos,
				&TotalFileSize) != 2)
			return _error->Error(
					_("The HTTP server sent an invalid Content-Range header"));
 ...
 */
BaseHttpMethod::DealWithHeadersResult BaseHttpMethod::DealWithHeaders(
		FetchResult &Res, RequestState &Req) {
	// Not Modified
	if (Req.Result == 304) {
	...
	constexpr unsigned int RedirectCodes[] = { 301, // Moved Permanently
			302, // Found
			303, // See Other
	...
	if (AllowRedirect
			&& std::find(std::begin(RedirectCodes), std::end(RedirectCodes),
					Req.Result) != std::end(RedirectCodes)) {
		if (Req.Location.empty() == true)
			...
			if (tmpURI.Access.find('+') != std::string::npos) {
				_error->Error(
						"Server tried to trick us into using a specific implementation: %s",
						tmpURI.Access.c_str());
				if (Req.HaveContent == true)
					return ERROR_WITH_CONTENT_PAGE;
				return ERROR_UNRECOVERABLE;
			...
			else if ((Uri.Access == "http" || Uri.Access == "https+http")
					&& tmpURI.Access == "https")
				return TRY_AGAIN_OR_REDIRECT;
			else {
				auto const tmpplus = tmpURI.Access.find('+');
				if (tmpplus != std::string::npos
						&& tmpURI.Access.substr(tmpplus + 1) == "https") {
					...
					if (uriplus == std::string::npos) {
						if (Uri.Access == tmpURI.Access.substr(0, tmpplus)) // foo -> foo+https
							...
							&& Uri.Access.substr(0, uriplus)
									== tmpURI.Access.substr(0, tmpplus)) // foo+http -> foo+https
						return TRY_AGAIN_OR_REDIRECT;
				}
			}
			_error->Error("Redirection from %s to '%s' is forbidden",
					Uri.Access.c_str(), NextURI.c_str());
		}
	...
	// retry after an invalid range response without partial data
		...
		if (stat(Queue->DestFile.c_str(), &SBuf) >= 0 && SBuf.st_size > 0) {
			...
			if (Queue->ExpectedHashes.usable() == true) {
				...
				HashStringList const hashList =
						resultHashes.GetHashStringList();
		...
		if (UsableHashes && FileExists(QueueBack->DestFile)) {
			...
			if (QueueBack->ExpectedHashes.FileSize() == partial.FileSize()) {
				if (wehave.AddFD(partial)
						&& wehave.GetHashStringList()
								== QueueBack->ExpectedHashes) {
					FetchResult Res;
	...
	while (1) {
		...
		if (Server == 0 || Server->Comp(Queue->Uri) == false) {
			if (!Queue->Proxy().empty()) {
				URI uri = Queue->Uri;
				_config->Set("Acquire::" + uri.Access + "::proxy::" + uri.Host,
						Queue->Proxy());
			}
		...
		/* If the server has explicitly said this is the last connectionOF
		...
		case FILE_IS_OPEN: {
			...
			if (Req.HaveContent) {
				...
				if (Req.DownloadSize > 0) {
					decltype(Queue->ExpectedHashes.FileSize()) const filesize =
							Req.StartPos + Req.DownloadSize;
					bool found = false;
					for (FetchItem const *I = Queue; I != 0 && I != QueueBack;
							I = I->Next) {
			...
			if (Result == ResultState::SUCCESSFUL) {
				...
				HashStringList const hashList =
						resultHashes->GetHashStringList();
				if (PipelineDepth != 0 && Queue->ExpectedHashes.usable() == true
						&& Queue->ExpectedHashes != hashList) {
					// we did not get the expected hash… mhhh:
					// could it be that server/proxy messed up pipelining?
					FetchItem * BeforeI = Queue;
					for (FetchItem *I = Queue->Next; I != 0 && I != QueueBack;
							I = I->Next) {
						if (I->ExpectedHashes.usable() == true
								&& I->ExpectedHashes == hashList) {
		...
		case ERROR_WITH_CONTENT_PAGE: {
			Server->RunDataToDevNull(Req);
			constexpr unsigned int TransientCodes[] = { 408, // Request Timeout
			...
			if (std::find(std::begin(TransientCodes), std::end(TransientCodes),
					Req.Result) != std::end(TransientCodes))
				Fail(true);
			else
				Fail();
			break;
...
/*}}}*/
BaseHttpMethod::BaseHttpMethod(std::string &&Binary, char const * const Ver,
		unsigned long const Flags) :/*{{{*/
		aptAuthConfMethod(std::move(Binary), Ver, Flags), Server(nullptr), PipelineDepth(
				10), AllowRedirect(false), Debug(false) {
}
/*}}}*/
...
bool BaseHttpMethod::AddProxyAuth(URI &Proxy, URI const &Server) /*{{{*/
{
	...
	if (std::find(methodNames.begin(), methodNames.end(), "tor")
			!= methodNames.end() && Proxy.User == "apt-transport-tor"
			&& Proxy.Password.empty()) {
		std::string pass = Server.Host;
		pass.erase(
				std::remove_if(pass.begin(), pass.end(),
